-- LibUIDropDownMenu, vendored for DH-Tools
-- Forked from Questie's own fork:
-- https://github.com/Questie/Questie/blob/master/Libs/LibUIDropDownMenu/LibUIDropDownMenu.lua
-- which itself forks the standard LibUIDropDownMenu-4.0 under a private
-- LibStub name (LibUIDropDownMenuDHTools-4.0) and private global frame
-- names (L_DropDownListDHTools1..N) so this copy can never collide with
-- another addon's copy of the same library via LibStub's shared registry.
-- See claude/knowledge/ for the write-up on why this matters (DH-Air's
-- unreproducible right-click menu bug).

-- This library is forked for DHTools into different names to avoid using
-- versions modified by other addons.
-- Fixed upstream bug.
-- For searching string/name to replace, still lot of falses:
-- (?<!(DHTools)|(Lib:UI)|(Lib:Close)|(Lib:Toggle)|(\\UI-))dropdown(?!.*DHTools)
...
-- ----------------------------------------------------------------------------
local MAJOR_VERSION = "LibUIDropDownMenuDHTools-4.0"
...
-- //////////////////////////////////////////////////////////////
L_UIDROPDOWNMENUDHTOOLS_MINBUTTONS = 8; -- classic only
L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS = 1;
L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS = 3;
L_UIDROPDOWNMENUDHTOOLS_BUTTON_HEIGHT = 16;
L_UIDROPDOWNMENUDHTOOLS_BORDER_HEIGHT = 15;
-- The current open menu
L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU = nil;
-- The current menu being initialized
L_UIDROPDOWNMENUDHTOOLS_INIT_MENU = nil;
-- Current level shown of the open menu
L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL = 1;
-- Current value of the open menu
L_UIDROPDOWNMENUDHTOOLS_MENU_VALUE = nil;
-- Time to wait to hide the menu
L_UIDROPDOWNMENUDHTOOLS_SHOW_TIME = 2;
-- Default dropdown text height
L_UIDROPDOWNMENUDHTOOLS_DEFAULT_TEXT_HEIGHT = nil;
-- For Classic checkmarks, this is the additional padding that we give to the button text.
L_UIDROPDOWNMENUDHTOOLS_CLASSIC_CHECK_PADDING = 4;
-- List of open menus
L_OPEN_DHTOOLSDROPDOWNMENUS = {};
...
delegateFrame:SetScript("OnAttributeChanged", function(self, attribute, value)
    ...
    elseif (attribute == "initmenu") then
        L_UIDROPDOWNMENUDHTOOLS_INIT_MENU = value;
    elseif (attribute == "openmenu") then
        L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU = value;
    end
end);

function lib:UIDropDownMenu_InitializeHelper(frame)
    -- This deals with the potentially tainted stuff!
    if (frame ~= L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU) then
        L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL = 1;
    end
    ...
    for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS, 1 do
        dropDownList = _G["L_DropDownListDHTools" .. i];
        if (i >= L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL or frame ~= L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU) then
            dropDownList.numButtons = 0;
            dropDownList.maxWidth = 0;
            for j = 1, L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS, 1 do
                button = _G["L_DropDownListDHTools" .. i .. "Button" .. j];
                button:Hide();
            end
            dropDownList:Hide();
        end
    end
    frame:SetHeight(L_UIDROPDOWNMENUDHTOOLS_BUTTON_HEIGHT * 2);
end

--DHTools addition, to know when menus are showing
function lib:getOpen() 
    return L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU ~= nil
end
...
local function create_MenuButton(name, parent)
    ...
    local function button_OnEnter(self)
        if (self.hasArrow) then
            ...
            local listFrame = _G["L_DropDownListDHTools" .. level];
...
local function creatre_DropDownList(name, parent)
    ...
    f:SetScript("OnShow", function(self)
        ...
        for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS do
        ...
        if (self:GetID() > 1) then
            self.parent = _G["L_DropDownListDHTools" .. (self:GetID() - 1)];
        end
    ...
    f:SetScript("OnHide", function(self)
        ...
        L_OPEN_DHTOOLSDROPDOWNMENUS[id] = nil;
        if (id == 1) then
            L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU = nil;
        end
...
local function create_DropDownButtons()
    L_DropDownList1 = creatre_DropDownList("L_DropDownListDHTools1")
    ...
    local _, fontHeight, _ = _G["L_DropDownListDHTools1Button1NormalText"]:GetFont()
    L_UIDROPDOWNMENUDHTOOLS_DEFAULT_TEXT_HEIGHT = fontHeight

    L_DropDownList2 = creatre_DropDownList("L_DropDownListDHTools2")
    ...
    L_DropDownList3 = creatre_DropDownList("L_DropDownListDHTools3")
    ...
    --tinsert(UIMenus, "L_DropDownListDHTools1");
    --tinsert(UIMenus, "L_DropDownListDHTools2");
    --tinsert(UIMenus, "L_DropDownListDHTools3");
...
function lib:UIDropDownMenu_Initialize(frame, initFunction, displayMode, level, menuList)
    ...
    local dropDownList = _G["L_DropDownListDHTools" .. level];
...
function lib:UIDropDownMenu_RefreshDropDownSize(self)
    ...
    for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS, 1 do
...
function lib:UIDropDownMenu_StartCounting(frame)
    if (frame.parent) then
        ...
        frame.showTimer = L_UIDROPDOWNMENUDHTOOLS_SHOW_TIME;
        frame.isCounting = 1;
    end
end
...
info.value = [ANYTHING]  --  The value that L_UIDROPDOWNMENUDHTOOLS_MENU_VALUE is set to when the button is clicked
...
function lib:UIDropDownMenu_CreateFrames(level, index)
    while (level > L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS) do
        L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS = L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS + 1;
        --local newList = CreateFrame("Button", "L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS, nil, "L_UIDropDownListTemplate");
        local newList = creatre_DropDownList("L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS)
        newList:SetFrameStrata("FULLSCREEN_DIALOG");
        newList:SetToplevel(true);
        newList:Hide();
        newList:SetID(L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS);
        newList:SetWidth(180)
        newList:SetHeight(10)
        --for i = WoWRetail and 1 or (L_UIDROPDOWNMENUDHTOOLS_MINBUTTONS+1), L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS do
        for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS do
            --local newButton = CreateFrame("Button", "L_DropDownListDHTools" .. L_UIDROPDOWNMENU_MAXLEVELS .. "Button" .. i, newList, "L_UIDropDownMenuButtonTemplate");
            local newButton = create_MenuButton("L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS .. "Button" .. i, newList)
            newButton:SetID(i);
        end
    end

    while (index > L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS) do
        L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS = L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS + 1;
        for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS do
            --local newButton = CreateFrame("Button", "L_DropDownListDHTools"..i.."Button"..L_UIDROPDOWNMENU_MAXBUTTONS, _G["L_DropDownListDHTools" .. i], "L_UIDropDownMenuButtonTemplate");
            local newButton = create_MenuButton("L_DropDownListDHTools" .. i .. "Button" .. L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS, _G["L_DropDownListDHTools" .. i])
            newButton:SetID(L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS);
        end
    end
end
...
function lib:UIDropDownMenu_AddButton(info, level)
	...
	if (not L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU) then
		return;
	end
	]]
    if (not level) then
    ...
    listFrame = listFrame or _G["L_DropDownListDHTools" .. level];
    ...
    local xPos = 5;
    local buttonHeight = (info.topPadding or 0) + L_UIDROPDOWNMENUDHTOOLS_BUTTON_HEIGHT;
    local yPos = -((button:GetID() - 1) * buttonHeight) - L_UIDROPDOWNMENUDHTOOLS_BORDER_HEIGHT;
    ...
    local frame = L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU;
    ...
    frame = frame or L_UIDROPDOWNMENUDHTOOLS_INIT_MENU;
    ...
    if not info.notCheckable then
        ...
        elseif info.classicChecks then
            ...
            normalText:SetPoint("LEFT", check, "RIGHT", L_UIDROPDOWNMENUDHTOOLS_CLASSIC_CHECK_PADDING, 0);
    ...
    if (info.hasColorSwatch) then
        if (WoWClassicEra or WoWClassicTBC or WoWWOTLKC) then
            _G["L_DropDownListDHTools" .. level .. "Button" .. index .. "ColorSwatch" .. "NormalTexture"]:SetVertexColor(info.r, info.g, info.b);
        else
            _G["L_DropDownListDHTools" .. level .. "Button" .. index .. "ColorSwatch"].Color:SetVertexColor(info.r, info.g, info.b);
        end
        button.r = info.r;
        button.g = info.g;
        button.b = info.b;
    ...
    if (WoWRetail) then
        ...
        local height = ((index - customFrameCount) * buttonHeight) + (L_UIDROPDOWNMENUDHTOOLS_BORDER_HEIGHT * 2);
        ...
        -- Set the height of the listframe
        listFrame:SetHeight((index * L_UIDROPDOWNMENUDHTOOLS_BUTTON_HEIGHT) + (L_UIDROPDOWNMENUDHTOOLS_BORDER_HEIGHT * 2));
    end
...
function lib:UIDropDownMenu_GetButtonWidth(button)
    ...
    elseif (normalText and normalText:GetText()) then
        ...
        if (button.classicChecks) then
            width = width + L_UIDROPDOWNMENUDHTOOLS_CLASSIC_CHECK_PADDING;
        end
    else
        return minWidth;
    end
...
function lib:UIDropDownMenu_Refresh(frame, useValue, dropdownLevel)
    ...
    if (not dropdownLevel) then
        dropdownLevel = L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL;
    end

    local listFrame = _G["L_DropDownListDHTools" .. dropdownLevel];
    listFrame.numButtons = listFrame.numButtons or 0;
    -- Just redraws the existing menu
    for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS do
        local button = _G["L_DropDownListDHTools" .. dropdownLevel .. "Button" .. i];
        local checked = nil;
        ...
        if not button.notCheckable and button:IsShown() then
            ...
            local checkImage = _G["L_DropDownListDHTools" .. dropdownLevel .. "Button" .. i .. "Check"];
            local uncheckImage = _G["L_DropDownListDHTools" .. dropdownLevel .. "Button" .. i .. "UnCheck"];
            if (checked) then
    ...
    if (not frame.noResize) then
        ...
        lib:UIDropDownMenu_RefreshDropDownSize(_G["L_DropDownListDHTools" .. dropdownLevel]);
    end
end

function lib:UIDropDownMenu_RefreshAll(frame, useValue)
    for dropdownLevel = L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL, 2, -1 do
        local listFrame = _G["L_DropDownListDHTools" .. dropdownLevel];
...
function lib:UIDropDownMenu_GetSelectedID(frame)
    if (frame.selectedID) then
        ...
        --[[		local maxNum;
		if (WoWClassicEra or WoWClassicTBC or WoWWOTLKC) then
			maxNum = L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS
		else
			local listFrame = _G["L_DropDownListDHTools"..L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL];
			maxNum = listFrame.numButtons
		end
		for i=1, maxNum do]]
        local listFrame = _G["L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL];
        for i = 1, listFrame.numButtons do
            local button = _G["L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL .. "Button" .. i];
            -- See if checked or not
            if (lib:UIDropDownMenu_GetSelectedName(frame)) then
                if (button:GetText() == lib:UIDropDownMenu_GetSelectedName(frame)) then
                    return i;
...
function lib:ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
    ...
    delegateFrame:SetAttribute("createframes", true);
    L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL = level;
    L_UIDROPDOWNMENUDHTOOLS_MENU_VALUE = value;
    local listFrameName = "L_DropDownListDHTools" .. level;
    ...
    if (listFrame:IsShown() and (L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU == tempFrame)) then
        listFrame:Hide();
    else
        ...
        if (level == 1) then
            ...
            if (not anchorName) then
                ...
                if (dropDownFrame.relativeTo) then
                    ...
                    relativeTo = GetChild(L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU, L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU:GetName(), "Left");
                end
            ...
            if (not dropDownFrame) then
                dropDownFrame = L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU;
            end
            listFrame:ClearAllPoints();
            -- If this is a dropdown button, not the arrow anchor it to itself
            if (strsub(button:GetParent():GetName(), 1, 21) == "L_DropDownListDHTools" and strlen(button:GetParent():GetName()) == 22) then
                anchorFrame = button;
            else
        ...
        if (level == 1) then
            ...
            listFrame:ClearAllPoints();
            listFrame.parentLevel = tonumber(strmatch(anchorFrame:GetName(), "L_DropDownListDHTools(%d+)"));
...
function lib:CloseDropDownMenus(level)
    ...
    for i = level, L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS do
        _G["L_DropDownListDHTools" .. i]:Hide();
    end
...
local function containsMouse()
    ...
    for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXLEVELS do
        local dropdown = _G["L_DropDownListDHTools" .. i];
    ...
    if L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU and L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU.Button:IsMouseOver() then
        result = true;
    end
...
function lib:UIDropDownMenu_ClearAll(frame)
    ...
    for i = 1, L_UIDROPDOWNMENUDHTOOLS_MAXBUTTONS do
        button = _G["L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL .. "Button" .. i];
        button:UnlockHighlight();

        checkImage = _G["L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL .. "Button" .. i .. "Check"];
        checkImage:Hide();
        uncheckImage = _G["L_DropDownListDHTools" .. L_UIDROPDOWNMENUDHTOOLS_MENU_LEVEL .. "Button" .. i .. "UnCheck"];
        uncheckImage:Hide();
    end
end
...
function lib:UIDropDownMenu_GetCurrentDropDown()
    if (L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU) then
        return L_UIDROPDOWNMENUDHTOOLS_OPEN_MENU;
    elseif (L_UIDROPDOWNMENUDHTOOLS_INIT_MENU) then
        return L_UIDROPDOWNMENUDHTOOLS_INIT_MENU;
    end
end
...
function lib:UIDropDownMenuButton_OpenColorPicker(self, button)
    ...
    L_UIDROPDOWNMENUDHTOOLS_MENU_VALUE = button.value;
    lib:OpenColorPicker(button);
end

function lib:UIDropDownMenu_DisableButton(level, id)
    lib:UIDropDownMenu_SetDropdownButtonEnabled(_G["L_DropDownListDHTools" .. level .. "Button" .. id], false);
end

function lib:UIDropDownMenu_EnableButton(level, id)
    lib:UIDropDownMenu_SetDropdownButtonEnabled(_G["L_DropDownListDHTools" .. level .. "Button" .. id], true);
end
...
function lib:UIDropDownMenu_SetButtonText(level, id, text, colorCode)
    local button = _G["L_DropDownListDHTools" .. level .. "Button" .. id];
    if (colorCode) then
...
function lib:UIDropDownMenu_SetButtonNotClickable(level, id)
    _G["L_DropDownListDHTools" .. level .. "Button" .. id]:SetDisabledFontObject(GameFontHighlightSmallLeft);
end

function lib:UIDropDownMenu_SetButtonClickable(level, id)
    _G["L_DropDownListDHTools" .. level .. "Button" .. id]:SetDisabledFontObject(GameFontDisableSmallLeft);
end
...
function lib:UIDropDownMenu_GetValue(id)
    ...
    local button = _G["L_DropDownListDHTools1Button" .. id];
    if (button) then
        return _G["L_DropDownListDHTools1Button" .. id].value;
    else
        return nil;
    end
end
